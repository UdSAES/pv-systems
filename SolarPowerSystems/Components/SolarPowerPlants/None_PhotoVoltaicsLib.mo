...
model None_PhotoVoltaicsLib
  ...
  extends Interfaces.PhotoVoltaicPowerPlant(redeclare Components.PlantInEnvironment.None inclinationAndShadowing(arrayTilt
        =arrayTilt, arrayAzimuth=arrayAzimuth),  redeclare Components.PhotoVoltaicArray.ModuleBased.PhotoVoltaicsLib plantIrradianceNormal(
      useHeatPort=true,
...
//     annotation (Placement(transformation(extent={{64,-90},{84,-70}})));
  ...
  PhotoVoltaics.Components.Blocks.MPTrackerSample mpTracker(
    ...
    ImpRef=plantIrradianceNormal.npModule*plantIrradianceNormal.moduleData.ImpRef)                                                                           annotation (
...
equation
  ...
  connect(converter.ac_n, groundAC.pin) annotation (Line(points={{80,-10},{80,-18},{90,-18}}, color={85,170,255}));

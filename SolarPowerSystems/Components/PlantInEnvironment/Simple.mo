...
//   P = panel_area * overall_efficiency * total_irra_on_panel * eta_mod;
  ...
  diffuse_irra_on_panel = noEvent(max(0, I_diff_horizontal  * 0.5 * (1 + cos(panel_slope))));

  ref_irra_on_panel = surfaceAlbedo * (I_dir_horizontal + I_diff_horizontal) * 0.5 * (1 - cos(panel_slope));

...
model IsotropicFromDNI
  "Transform DNI and DHI to normal of arbitrarily oriented surface using the `Buildings`-library"
  extends SolarPowerSystems.Interfaces.PlantInEnvironment;
  Buildings.BoundaryConditions.SolarIrradiation.BaseClasses.DiffuseIsotropic diffuse(rho=0.2, til=arrayTilt)
    annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=270,
        origin={-4,0})));
  Buildings.BoundaryConditions.SolarIrradiation.BaseClasses.DirectTiltedSurface direct annotation (Placement(
        transformation(
        extent={{-10,-10},{10,10}},
        rotation=270,
        origin={60,0})));
...
equation
  ...
  connect(direct.HDirTil, directInclinedIrradiance)
    annotation (Line(points={{60,-11},{60,-100}}, color={0,0,127}));
  ...
  connect(directNormalIrradiance, direct.HDirNor)
    annotation (Line(points={{100,40},{66,40},{66,12}}, color={0,0,127}));

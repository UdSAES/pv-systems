within SolarPowerSystems.Components.SolarPosition.UnitTests;
model SolarPositionVsIrradiance
  ...
  SolarPowerSystems.Components.SolarPosition.SolarAzimuth solarAzimuth(
    latitude=location.latitude,
    longitude=location.longitude,
    simu_start_epochs=epochOffset.k,
    height_above_sealevel=location.elevation)
    annotation (Placement(transformation(extent={{70,10},{90,30}})));
  SolarPowerSystems.Components.SolarPosition.SolarZenith solarZenith(
    latitude=location.latitude,
    longitude=location.longitude,
    simu_start_epochs=epochOffset.k,
    height_above_sealevel=location.elevation)

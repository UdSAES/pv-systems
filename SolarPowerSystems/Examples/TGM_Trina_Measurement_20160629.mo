...
model TGM_Trina_Measurement_20160629
  ...
  Components.SolarPowerPlant.AreaBased.AreaBased plantModelAreaBased(
      overall_efficiency=0.141, panel_area=1.650*0.992*8) annotation (Placement(
        transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={0,-60})));
equation
  connect(mpTracker.vRef, converter.vDCRef) annotation (
    Line(points={{21,20},{24,20},{24,58}},           color = {0, 0, 127}));
  ...
  connect(ground.p, converter.dc_n) annotation (
    Line(points={{-40,50},{20,50},{20,60}},           color = {0, 0, 255}));
  connect(inputData.y[1], plantModelModuleBased.I_G_normal) annotation (Line(
        points={{-69,0},{-60,0},{-60,70},{-50,70}}, color={0,0,127}));
  connect(plantModelModuleBased.n1, ground.p)
    annotation (Line(points={{-40,60},{-40,50}}, color={0,0,255}));
  connect(plantModelModuleBased.p1, converter.dc_p) annotation (Line(points={{
          -40,80},{-40,90},{20,90},{20,80}}, color={0,0,255}));
  connect(plantModelModuleBased.nv1, converter.dc_n) annotation (Line(points={{
          -30,75},{-28,75},{-28,50},{20,50},{20,60}}, color={0,0,255}));
  connect(plantModelModuleBased.P_DC, mpTracker.power) annotation (Line(points=
          {{-30,70},{-14,70},{-14,20},{-2,20}}, color={0,0,127}));

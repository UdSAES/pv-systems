...
partial model PhotoVoltaicPowerPlant
  ...
  parameter Boolean useWindSpeedInput=false "=true, if wind speed is provided"
    annotation (
    Evaluate=true,
    HideResult=true,
    choices(checkBox=true));
  parameter Modelica.SIunits.Velocity constWindSpeed=1 "Fixed wind speed value if useWindSpeedInput = false"
    annotation (Dialog(enable=not useWindSpeedInput));
  Modelica.Blocks.Sources.Constant fixedWindSpeed(y(unit="m/s"), final k=constWindSpeed) if not useWindSpeedInput
    annotation (Placement(transformation(extent={{-64,-44},{-56,-36}})));

  parameter Real latitude(
    unit="deg",
    min=-90,
    max=90) "Latitude in decimal degrees" annotation (Dialog(group="Location"));
  parameter Real longitude(
    unit="deg",
    min=-180,
    max=180) "Longitude in decimal degrees" annotation (Dialog(group="Location"));
  parameter Real elevation(
    unit="m",
    min=0,
    max=8848) "Height above sea level (elevation) in metres" annotation (Dialog(group="Location"));
  parameter Modelica.SIunits.Angle arrayTilt(min=0, max=90)
    "Array tilt in degree (horizontal equals 0°, vertical equals 90°)" annotation (Dialog(group="PV Plant"));
  ...
  replaceable PlantInEnvironment inclinationAndShadowing(
    arrayTilt=arrayTilt,
    arrayAzimuth=arrayAzimuth,
    albedo=albedo) constrainedby PlantInEnvironment "Select model to account for inclination and shadowing"
    annotation (
    ...
    Dialog(group="PV Plant", enable=false));
  ...
  replaceable PhotoVoltaicArray plantIrradianceNormal(useHeatPort=true) constrainedby PhotoVoltaicArray
  ...
  Components.SolarPosition.SolarPositionAlgorithm.SolarAzimuth solarAzimuth(
    ...
    simu_start_epochs=epochOffset) annotation (Placement(transformation(extent={{-60,72},{-40,92}})));
  Components.SolarPosition.SolarPositionAlgorithm.SolarZenith solarZenith(
    latitude=latitude,
    longitude=longitude,
    height_above_sealevel=elevation,
    simu_start_epochs=epochOffset) annotation (Placement(transformation(extent={{-60,50},{-40,70}})));
  Modelica.Thermal.HeatTransfer.Sources.PrescribedTemperature environmentTemperature if
                              useTemperatureInput "The temperature of the surrounding air"
  ...
  Modelica.Blocks.Interfaces.RealInput windSpeed if useWindSpeedInput
    "The wind speed at the plant's site (optional input)" annotation (Placement(transformation(
        extent={{-20,-20},{20,20}},
        rotation=0,
        origin={-100,-30})));

  Modelica.Blocks.Math.Max max annotation (Placement(transformation(extent={{-64,36},{-56,44}})));
  Modelica.Blocks.Math.Max max1 annotation (Placement(transformation(extent={{-64,-4},{-56,4}})));
  Modelica.Blocks.Sources.Constant const(k=0) annotation (Placement(transformation(extent={{-56,16},{-64,24}})));
  Modelica.Blocks.Math.Add add(k1=-1, k2=+1) annotation (Placement(transformation(extent={{76,-84},{84,-76}})));
  Modelica.Blocks.Sources.Constant transformFactor(k=90) "Factor to make angle positive iff sun is above horizon"
    annotation (Placement(transformation(extent={{54,-92},{62,-84}})));
...
equation
  connect(totalEnergyDC, integrator.y) annotation (Line(points={{100,40},{85,40}}, color={0,0,127}));
  connect(globalIrradiance.y, plantIrradianceNormal.I_G_normal)
    annotation (Line(points={{8.8,0},{22,0}}, color={0,0,127}));
  connect(solarAzimuth.y, inclinationAndShadowing.solarAzimuth)
    annotation (Line(points={{-39,82.2},{-26,82.2},{-26,10}}, color={0,0,127}));
  connect(solarZenith.y, inclinationAndShadowing.solarZenith)
    annotation (Line(points={{-39,60.2},{-30,60.2},{-30,10}}, color={0,0,127}));
  connect(temperature, environmentTemperature.T)
    annotation (Line(points={{-100,-80},{-64.8,-80}}, color={0,0,127}));
  ...
  connect(const.y, max.u2) annotation (Line(points={{-64.4,20},{-68,20},{-68,37.6},{-64.8,37.6}}, color={0,0,127}));
  connect(const.y, max1.u1) annotation (Line(points={{-64.4,20},{-68,20},{-68,2.4},{-64.8,2.4}}, color={0,0,127}));
  connect(max.y, inclinationAndShadowing.directHorizontalIrradiance)
    annotation (Line(points={{-55.6,40},{-50,40},{-50,6},{-42,6}}, color={0,0,127}));
  connect(max1.y, inclinationAndShadowing.diffuseHorizontalIrradiance)
    annotation (Line(points={{-55.6,0},{-42,0}}, color={0,0,127}));
  connect(diffuseHorizontalIrradiance, max1.u2)
    annotation (Line(points={{-100,30},{-72,30},{-72,-2.4},{-64.8,-2.4}}, color={0,0,127}));
  connect(directHorizontalIrradiance, max.u1)
    annotation (Line(points={{-100,80},{-68,80},{-68,42.4},{-64.8,42.4}}, color={0,0,127}));
  connect(solarZenith.y, add.u1)
    annotation (Line(points={{-39,60.2},{56,60.2},{56,-77.6},{75.2,-77.6}}, color={0,0,127}));
  connect(transformFactor.y, add.u2)
